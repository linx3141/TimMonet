...
import com.timmonet.core.BgResolver
import com.timmonet.core.ColorMath
...
object TimMonetHooks {
    ...
    /** 宿主包名（按名字查资源 id 时用）。 */
    private const val HOST_PACKAGE = "com.tencent.tim"
    private const val QQ_THEME = "com.tencent.mobileqq.utils.QQTheme"

    /** TIM 的"浅色（日间）"主题 id —— 取自反编译 ThemeUtil.java 的常量表
     *  THEME_ID_DAYMODE_SIMPLE_WHITE。注意不要用 DEFAULT_THEME_ID("1000")：
     *  那只是"默认/未设置"，不是浅色主题。 */
    ...
    /** ConcurrentHashMap 不接受 null 值：以前直接 `cache[key] = field/null` 会抛
     *  NPE（被上层 catch 静默吞掉），结果是"每次都要重新查 + 零日志"。
     *  用哨兵对象把"查过、确实没有"也缓存下来。 */
    private val cacheMiss = Any()
    private val fieldCache = ConcurrentHashMap<String, Any>()
    private val typeFieldCache = ConcurrentHashMap<String, Any>()
    private val methodCache = ConcurrentHashMap<String, Method?>()
    private val entryNameCache = ConcurrentHashMap<Long, Any>()
    ...
    /**
     * 附标所在行的卡片色缓存：View → (文本 hash, 颜色)。
     *
     * 键是 View **实例**（弱引用）。以前键是
     * `identityHashCode(view) xor text.hashCode()` —— 两个问题：
     * ① View 回收后哈希被复用 → 新行拿到**别的行**的卡片色；
     * ② xor 折叠把 (view, text) 两个输入压成一个 Int，不同组合会撞键。
     * 顺带不需要容量上限与代次清理：条目随 View 消失。
     */
    private val badgeRowCache: MutableMap<Any, Pair<Int, Int>> = ColorMath.instanceCache()
    ...
    private fun injectSettingEntry(list: MutableList<*>, ctx: Context, cl: ClassLoader) {
        ...
        // ⚠️ 条目/分组的类型获取方式（新版 TIM 改了结构，这里曾整个失效）：
        //
        //   旧做法：从"第一组的第一个 processor"反射取类型。以前那个位置恰好是
        //   一个通用条目类，现在变成了**具体项** —— setting.main.processor.a 就是
        //   "账号管理"（构造只有 Context、点击跳 AccountManageActivity），于是
        //   getConstructor(Context,int,CharSequence,int) 抛 NoSuchMethodException，
        //   整个注入静默失败（实测日志：setting entry inject failed: NoSuchMethodException
        //   ...processor.a.<init> [Context, int, CharSequence, int]）。
        //   这也解释了"只有装了 TAssistant/QAuxiliary 才看得到入口"——它们也往这个
        //   列表里插条目，列表结构被改后旧逻辑才偶尔能取到合适类型。
        //
        //   现在改为**直接使用 TIM 自带的两个通用类**（签名与我们需要的完全一致）：
        //     com.tencent.mobileqq.setting.processor.h(Context,int,CharSequence,int)  条目
        //     com.tencent.mobileqq.setting.processor.c(List,CharSequence,CharSequence) 分组
        //   取不到时再回退旧逻辑（抗混淆/老版本）。
        var itemCls: Class<*>? = runCatching {
            Class.forName("com.tencent.mobileqq.setting.processor.h", false, cl)
        }.getOrNull()
        var groupCls: Class<*>? = runCatching {
            Class.forName("com.tencent.mobileqq.setting.processor.c", false, cl)
        }.getOrNull()
        if (itemCls == null || groupCls == null) {
            val firstGroup = groups.firstOrNull()
            if (firstGroup != null) {
                if (groupCls == null) groupCls = firstGroup.javaClass
                if (itemCls == null) {
                    val processors = runCatching {
                        firstGroup.javaClass.getMethod("b").invoke(firstGroup) as? List<*>
                    }.getOrNull()
                    itemCls = processors?.firstOrNull()?.javaClass
                }
            }
        }
        if (itemCls == null || groupCls == null) {
            logOnce("setting entry inject skipped: itemCls/groupCls not found")
            return
        }
    ...
    /**
     * 按 constantState 共享判定结果（TIM 会复制 drawable 再交给 ImageView，
     * 复制品共享 constantState，所以按实例缓存会重复做整图渲染 + 逐像素统计）。
     *
     * ⚠️ 键是 constantState **对象本身**（弱引用），不是它的 identityHashCode：
     * 用 Int 做键时，constantState 被回收后哈希会被新对象复用 —— 新图会直接拿到
     * **别的图**的判定结果（"这张图像不像品牌图"判错 → 该染的不染/不该染的染），
     * 且随 GC 时机变化、无法稳定复现。弱引用键随对象消失，也不需容量上限。
     */
    private val brandLikeStateMemo: MutableMap<Any, Boolean> = ColorMath.instanceCache()
    ...
    private fun isBrandLikeImage(drawable: Drawable): Boolean {
        ...
        if (state != null) brandLikeStateMemo[state] = result
        return result
    }
    ...
    private fun hookMineGrid(module: XposedModule, cl: ClassLoader) {
        ...
        findMethodStrict(cls, setOf("h"), itemType)
            ?.let { method ->
                ...
                module.hook(method).intercept { chain ->
                    ...
                    try {
                        ...
                        val scheme = MonetPalette.palette()
                        // 卡片底在 TIM 里普遍是**半透明叠加层**（浅色是 ~10% 白高光，
                        // 深色是 ~10% 白/黑叠加），本身没有独立底色 —— 叠在页面底上
                        // 就几乎同色，表现为"卡片对比度极低"（实测深色下页面底
                        // #2C1211、卡片 #2F1312，只差 3/255）。
                        //
                        // ⚠️ 这里原来**只在 !isDark 时处理**，注释写着"深色模式卡底
                        // 另有路径"——但那个路径并不存在，所以深色下卡片一直是半透明
                        // 的（用户反馈："设成浅色对比度正常，深色就低"）。
                        // 现在两种模式都做不透明化，保留圆角形状。
                        runCatching {
                            ...
                            // 浅色只处理"近白高光层"；深色下卡片底必定是暗的
                            // 半透明层，不需要近白判据（否则永远不命中）。
                            val shouldFix = rCol != 0 && alpha in 1..127 &&
                                (scheme.isDark || BgResolver.isNearWhite(opaqueColor(rCol)))
                            if (shouldFix) {
                                val cardCol = TokenMapper.bgCard()
                                if (rb is android.graphics.drawable.ColorDrawable) {
                                    rb.mutate()
                                    rb.color = cardCol
                                } else if (rb is android.graphics.drawable.GradientDrawable) {
    ...
    private fun hookArkDialogBg(module: XposedModule, cl: ClassLoader) {
        for (clsName in listOf(
            ...
            // show() 继承自 Dialog，不在 declaredMethods 里 —— 以前用带兜底的
            // findMethod 会挂到同参数的 dismiss 上（弹窗关闭时才染背景）。
            findMethodStrictDeep(cls, setOf("show"))
                ?.let { method ->
                    logOnce("hook installed: $clsName.show (ark dialog bg)")
                    runCatching { module.deoptimize(method) }
                    module.hook(method).intercept { chain ->
                        try {
                            forwardConfirmDialogMonetize(chain.thisObject as? Dialog)
                        } catch (t: Throwable) {
    ...
    private fun forwardConfirmDialogMonetize(dlg: Dialog?) {
        if (dlg == null) return
    ...
    private fun dialogMonetizePass(decor: View, dark: Boolean) {
        val scheme = MonetPalette.palette()
        runCatching {
            forceMonetSubtree(decor)
        }
        if (dark) {
            runCatching {
                ...
                if (bg != null) {
                    ColorMath.recolorInPlace(bg, scheme.surfaceBright)
                }
            }
        }
        runCatching {
            tintEmoButtonIn(decor)
        }
                ...
                if (v2 is ImageView) {
                    ...
                    if (iw2 in 1..96 && ih2 in 1..96) {
                        ...
                        if (colorLuma(op2) < LUMA_MID) {
                            runCatching {
                                ColorMath.recolorInPlace(d2, target)
                            }
                        }
                    }
                }
    ...
    private fun hookForwardArkConfirm(module: XposedModule, cl: ClassLoader) {
        ...
        if (w0Cls != null) {
            listOf("w0").forEach { mName ->
                findMethodStrict(w0Cls, setOf(mName), Context::class.java, Bundle::class.java)
                    ?.let { method ->
        ...
        listOf("buildConfirmDialog").forEach { mName ->
            findMethodStrict(cls, setOf(mName))
                ?.let { method ->
                        ...
                        try {
                            ...
                            if (dlg != null) {
                                ...
                                if (dlg.javaClass.name.contains("QQCustomDialog")) {
                                    forwardConfirmDialogMonetize(dlg)
                                }
                            }
                        } catch (t: Throwable) {
    ...
    private fun tintEmoButtonIn(decor: View): Boolean {
        var tinted = false
        runCatching {
            val scheme = MonetPalette.palette()
    ...
    private fun hookEmoticonToggleBtn(module: XposedModule, cl: ClassLoader) {
        ...
        findMethodStrict(cls, setOf("a0"), java.lang.Boolean.TYPE)
            ?.let { method ->
...
private fun monetizeForwardPopup(activity: Activity) {
    ...
    decor.post {
        try {
            ...
            if (rootBg != null && !trySetPaintFilter(rootBg, TokenMapper.bgList())) {
                try {
                    ColorMath.recolorInPlace(rootBg, TokenMapper.bgList())
                } catch (t: Throwable) {
                    // ignore
                }
            }
...
/**
 * 颜色转不透明(补 alpha 位)，用于亮度判断/取色比较 —— 转发 [ColorMath.opaque]。
 *
 * 保留这个名字只为不动 40 多处调用点；不要在别处再手写
 * `(c and 0x00FFFFFF) or 0xFF000000.toInt()`。
 */
private fun opaqueColor(c: Int): Int = ColorMath.opaque(c)

/**
 * 遍历 [root] 的子树，最多访问 [maxNodes] 个节点。
 *
 * ⚠️ [visit] **不支持剪枝**：它的返回类型是 Unit，而且实现是"先把子节点压队、
 * 再回调当前节点"。曾经这里写着"返回 true 可跳过子树"，与代码不符，容易误用 ——
 * 需要剪枝请在回调里自行判断。
 *
 * 名字里的 walk 不承诺顺序：实现用 `ArrayDeque.removeFirst()`，实际是**深度优先**
 * （历史上注释称"广度优先"，也是错的）。
 */
...
private fun forceMonetSubtree(view: View?) {
    if (view == null) return
    forceMonetSubtree(view, MonetPalette.isDarkNow(), MonetPalette.palette())
}
...
private fun mapPopupTextColor(color: Int, scheme: DynamicScheme): Int {
    ...
    // 保留原色的透明度：一律走 ColorMath.keepAlpha，不再手写 `and 0x00FFFFFF or (alpha shl 24)`
    // —— 那种写法少写一半就会变成"不透明纯黑"（见 AGENTS.md 坑 5）。
    if (isSchemeColor(opaque)) {
        // 已经是角色色：onPrimary 这类角色色不动；其余低亮度角色色换 onSurface 提对比
        if (opaque != ColorMath.opaque(scheme.onPrimary) &&
            colorLuma(opaque) < LUMA_MID_LOW
        ) {
            return ColorMath.keepAlpha(scheme.onSurface, color)
        }
        return color
    }
    ...
    val mapped = TokenMapper.mapColor(null, opaque)
...
private fun hookSplashBackground(module: XposedModule, cl: ClassLoader) {
    ...
    listOf("a", "b").forEach { mName ->
        findMethodStrict(cls, setOf(mName), Context::class.java)
            ?.let { method ->
                ...
                module.hook(method).intercept { chain ->
                    ...
                    try {
                        ...
                        if (d != null) {
                            ...
                            runCatching {
                                ColorMath.recolorInPlace(d, target)
                            }
                        }
                    } catch (t: Throwable) {
                        // ignore
...
private fun hookForwardDialog(module: XposedModule, cl: ClassLoader) {
    try {
        ...
        findMethodStrict(cls, setOf("setContentView"), INT_TYPE)
            ?.let { method ->
        ...
        findMethodStrict(cls, setOf("show"))
            ?.let { method ->
    ...
    try {
        ...
        findMethodStrict(pv, setOf("setEmoButtonImageResourceATag"), INT_TYPE, INT_TYPE)
            ?.let { method ->
...
private fun buildPlusItemPlate(density: Float): Drawable {
    ...
    val veil = ColorMath.withAlpha(scheme.onSurface, 0x24)
...
private fun fixPlusItemPlate(v: View) {
    ...
    if (ok.isSuccess) plusPlateViews.add(v)
}
...
/**
 * drawable 的 constantState → 资源名（复制品共享 constantState，用于兜底查名）。
 *
 * ⚠️ 键是 constantState **对象本身**（WeakHashMap），不是它的 identityHashCode：
 * 用 Int 做键时，constantState 被回收后哈希会被新对象复用，于是兜底查名会返回
 * **另一个 drawable 的资源名** —— 表现是"某个图标偶尔被染成完全不相干的角色的颜色"，
 * 且随 GC 时机变化。同时它以前没有容量上限、从不清空，会随进程一直增长。
                ...
                try {
                    ...
                    if (view != null) {
                        ...
                        view.background?.let { bg ->
                            runCatching {
                                ColorMath.recolorInPlace(bg, scheme.primary)
                            }
                        }
                        if (view is TextView) {
...
private val plusPanelClassMemo = java.util.concurrent.ConcurrentHashMap<Class<*>, Boolean>()
            ...
            module.hook(method).intercept { chain ->
                try {
                    ...
                    if (vg0 != null) {
                        runCatching {
                            ...
                            for (i in 0 until vg0.childCount) {
                                ...
                                if (c == TokenMapper.bgPage()) continue
                                if (BgResolver.luma(opaqueColor(c)) < BgResolver.LIGHT) continue
                                ColorMath.recolorInPlace(d, TokenMapper.bgPage())
                                if (thinLineLog++ < 12) {
                                    Log.i(
                                        TAG,
                    ...
                    if (vg != null && !isThirdPartyUiActive()) {
                        ...
                        if (isFullWidth(vg) && vg.height in vg.dpPx(32f)..vg.dpPx(48f)) {
...
private fun fixTitleBarIcon(v: View) {
    val fg = MonetPalette.palette().onSurface
    try {
        v.background?.let { bg ->
            ColorMath.recolorInPlace(bg, fg)
        }
        if (v is TextView) {
            for (d in v.compoundDrawables) {
                d?.let { ColorMath.recolorInPlace(it, fg) }
            }
        }
        if (titleBarLog++ < 6) {
            Log.i(
...
private fun tintBadgeDigitText(tv: TextView, scheme: DynamicScheme): Boolean {
    ...
    /** 是否与当前 primary 同色（逐通道容差）——容差常量统一由 [BgResolver] 定义。 */
    fun isPrimaryColor(c0: Int): Boolean {
        val op = opaqueColor(c0)
        val tol = BgResolver.ROLE_TOLERANCE
        return kotlin.math.abs((op ushr 16 and 0xFF) - (primFull ushr 16 and 0xFF)) < tol &&
            kotlin.math.abs((op ushr 8 and 0xFF) - (primFull ushr 8 and 0xFF)) < tol &&
            kotlin.math.abs((op and 0xFF) - (primFull and 0xFF)) < tol
    }
...
private fun hookResumeRefresh(module: XposedModule) {
...
private fun loginPageMonetizePass(decor: View) {
    try {
        val scheme = MonetPalette.palette()
        if (!scheme.isDark) return
            ...
            try {
                when (b) {
                    ...
                    is android.graphics.drawable.GradientDrawable -> {
                        ...
                        runCatching {
                            ColorMath.recolorInPlace(b, target)
                        }
                    }
                }
            } catch (t: Throwable) {
                        ...
                        try {
                            when (hb) {
                                ...
                                is android.graphics.drawable.ColorDrawable -> {
                                    ...
                                    ColorMath.recolorInPlace(hb, hi)
                                }
                            }
...
/** 遍历 drawable 容器（DrawableContainer / StateListDrawable / LayerDrawable…）
 *  的每个子 drawable 并回调 —— 这是文件里 8 个"容器重染"函数的**公共骨架**。
 *
 *  ⚠️ 只统一"怎么遍历"，**不统一"选哪些子项、染什么色"**：那部分各函数差异是
 *  有意义的（例如开关轨道按位置选色、通用容器只染近白子项），硬合并会改变行为。
 *
 *  @param recurse 是否递归进嵌套容器（默认 true）
 *  @param apply   对每个非空子项调用
 *  @return 是否至少处理了一个子项
 */
private fun forEachContainerChild(
    drawable: Drawable?,
    recurse: Boolean = true,
    apply: (Drawable) -> Unit
): Boolean {
    if (drawable == null) return false
    // LayerDrawable 不在 DrawableContainer 体系里，单独处理
    if (drawable is LayerDrawable) {
        var handledL = false
        for (i in 0 until runCatching { drawable.numberOfLayers }.getOrDefault(0)) {
            val layer = runCatching { drawable.getDrawable(i) }.getOrNull() ?: continue
            handledL = true
            if (recurse && layer is DrawableContainer) {
                forEachContainerChild(layer, true, apply)
            } else {
                apply(layer)
            }
        }
    ...
    val children = runCatching { state.children }.getOrNull() ?: return false
    var handled = false
    for (child in children) {
        if (child == null) continue
        handled = true
        if (recurse && child is DrawableContainer) {
            forEachContainerChild(child, true, apply)
        } else {
            apply(child)
        }
    }
...
private fun forceSolidDrawableColor(drawable: Drawable?, color: Int): Boolean {
    ...
    if (drawable is DrawableContainer || drawable is LayerDrawable) {
        // 遍历交给公共骨架（含嵌套与 LayerDrawable），本函数只负责"怎么染"
        val handled = forEachContainerChild(drawable, recurse = false) { child ->
            forceSolidDrawableColor(child, color)
        }
    ...
    try {
        ColorMath.recolorInPlace(drawable, color)
        drawable.invalidateSelf()
        return true
    } catch (t: Throwable) {
        return false
...
// ============================================================================
// ⚠️ 本段的两个函数是"从 Java 反编译恢复"的产物，**恢复不完整**，改动前务必注意：
//   - recolorReplyText：原 Java 里只剩 JADX 未反编译的占位，287 条指令正文缺失，
//     现在的实现是尽力还原，不保证与原版逐条等价；
//   - bubbleHost：原 Java 里是 JADX **寄存器级**部分 dump，按 dump 还原，终值有歧义。
// 两者都在活跃调用路径上（回复气泡染色），行为异常时优先怀疑这里。
// ============================================================================
...
private fun hookAlbumTimelineText(module: XposedModule, cl: ClassLoader) {
    try {
        ...
        findMethodStrict(cls, setOf("a"), INT_TYPE, String::class.java, INT_TYPE)
            ?.let { method ->
        ...
        try {
            ...
            findMethodStrict(cls, setOf("setPhotoList"), listType)
                ?.let { method ->
...
private fun forceTimelineText(view: View?, cl: ClassLoader) {
    if (view == null) return
    val palette = MonetPalette.palette()
    val primary = palette.primary
    val onPrimary = palette.onPrimary
    // ⚠️ 按**名字**取资源 id，不写死数字：id 每次 TIM 重打包/混淆都会变，
    // 写死一旦偏移就会把**别的 View** 染成 primary（视觉错乱），而不是"不生效"。
    // 反编译 public.xml 确认：2131308278 = id/qa2（气泡底）、2131308279 = id/qa3（日期文字）。
    val res = view.resources
    val idBubble = res.getIdentifier("qa2", "id", HOST_PACKAGE)
    val idDate = res.getIdentifier("qa3", "id", HOST_PACKAGE)
    if (view is ViewGroup) {
        val childCount = view.childCount
        for (i in 0 until childCount) {
            val child = view.getChildAt(i)
            if (idBubble != 0 && child.id == idBubble) {
                forceBubbleColor(child, primary)
            } else if (idDate != 0 && child.id == idDate && child is TextView) {
...
private fun forceBubbleColor(view: View, color: Int) {
    ...
    try {
        ColorMath.recolorInPlace(bg, color)
        view.invalidate()
        if (timelineRecolorCount++ < 10) {
            Log.i(TAG, "album timeline bubble bg -> #${Integer.toHexString(color)}")
        }
...
/** 强制 TIM 走浅色资源的**真正**入口。
 *
 *  TIM 的深浅用的是标准的 -night 资源限定符（res 里有 color-night /
 *  values-night / values-night-v29），也就是由 Configuration.uiMode 决定 ——
 *  TIM 那个「深色模式」开关最终就是把它设成 UI_MODE_NIGHT_YES，于是所有 night
 *  资源被选中，卡片底色变成深色那套，与我们的莫奈染色叠加后对比度很低。
 *
 *  ⚠️ 拦 ThemeUtil / QQTheme 的 isNowThemeIsNight 之类**根本没用**：那些方法只
 *  影响 TIM 自己的业务逻辑，不参与资源解析（实测 ThemeUtil 那个甚至从没被调用过）。
 *  必须在取 Configuration 时就把它改成"非夜间"。
 */
private fun hookForceLightConfig(module: XposedModule) {
    runCatching {
        val m = Resources::class.java.getDeclaredMethod("getConfiguration")
        runCatching { module.deoptimize(m) }
        module.hook(m).intercept { chain ->
            // ⚠️ 绝不能动 Resources.getSystem()：那是**系统全局**配置，而
            // MonetPalette.systemDark()（"自动"深浅模式）正是靠它判断的 ——
            // 改了会让"自动"永远落到浅色，等于模块自己的取色被 TIM 的设置绑住
            // （这正是历史上 isNight 那条链的同类问题）。
            // 我们只该改 TIM 自己的 Resources。
            if (chain.thisObject === Resources.getSystem()) return@intercept chain.proceed()
            val cfg = runCatching { chain.proceed() as? Configuration }.getOrNull()
            if (cfg != null &&
                (cfg.uiMode and Configuration.UI_MODE_NIGHT_MASK) ==
                Configuration.UI_MODE_NIGHT_YES
            ) {
                cfg.uiMode = (cfg.uiMode and Configuration.UI_MODE_NIGHT_MASK.inv()) or
                    Configuration.UI_MODE_NIGHT_NO
                if (forceLightCfgLog++ < 3) {
                    Log.i(TAG, "force light: uiMode NIGHT_YES -> NO")
                }
            }
            cfg
        }
        logOnce("hook installed: Resources.getConfiguration (force light)")
    }.onFailure { Log.w(TAG, "force light config hook failed", it) }
}

private fun hookForceLight(module: XposedModule, cl: ClassLoader) {
    try {
        val cls = Class.forName(DARK_MODE_MANAGER, false, cl)
        findMethodStrict(cls, setOf("f"))
            ?.let { method ->
    ...
    try {
        ...
        findMethodStrict(cls, setOf("isNowThemeIsNight"))
            ?.let { method ->
                logOnce("hook installed: $QQ_THEME.isNowThemeIsNight")
                module.hook(method).intercept { false }
            }
        findMethodStrict(cls, setOf("getCurrentThemeId"))
            ?.let { method ->
                logOnce("hook installed: $QQ_THEME.getCurrentThemeId")
                module.hook(method).intercept { THEME_ID_DAYMODE_SIMPLE_WHITE }
            }
        findMethodStrict(cls, setOf("isVasTheme"))
            ?.let { method ->
                logOnce("hook installed: $QQ_THEME.isVasTheme")
                module.hook(method).intercept { false }
            }
        // ⚠️ 下面两条是"强制浅色"此前漏掉的路径：
        // isLastThemeIsNight() 读的是 **lastThemeId 字段**（由 storeLastThemeId 写入），
        // 并不经过 getCurrentThemeId() —— 所以上面那些 hook 覆盖不到它。
        // theme.a(ThemeNavigationBarUtil) 会拿它和 isNowThemeIsNight() 一起决定
        // 导航栏颜色：一旦 lastThemeId 是 "1103"/"2920"（夜间主题）就会走进
        // "夜间导航栏"分支，把导航栏设成保存的浅色 f134368a。
        findMethodStrict(cls, setOf("isLastThemeIsNight"))
            ?.let { method ->
                logOnce("hook installed: $QQ_THEME.isLastThemeIsNight")
                module.hook(method).intercept { false }
            }
        findMethodStrict(cls, setOf("storeLastThemeId"), String::class.java)
            ?.let { method ->
                logOnce("hook installed: $QQ_THEME.storeLastThemeId")
                module.hook(method).intercept { chain ->
                    // 从源头把"上一个主题 id"也钉成浅色默认值
                    chain.proceed(arrayOf<Any>("2971"))
                }
            }
    } catch (t: Throwable) {
        Log.w(TAG, "QQTheme not found", t)
    }
    // ⚠️ 真正的大头：vas.theme.api.ThemeUtil.isNowThemeIsNight(AppRuntime, boolean, String)
    // —— TIM 里 **500+ 个类**调用它，而它内部走的是 ThemeUtil 自己的
    // getCurrentThemeId() / getUserCurrentThemeId()，**完全不经过** QQTheme，
    // 所以上面那些 hook 一个都挡不住它（"强制浅色没成功"的真正原因）。
    // 直接把所有同名重载钉成 false。
    try {
        val utilCls = Class.forName("com.tencent.mobileqq.vas.theme.api.ThemeUtil", false, cl)
        val overloads = runCatching { utilCls.declaredMethods }.getOrNull()
            ?.filter { it.name == "isNowThemeIsNight" }
            .orEmpty()
        for (m in overloads) {
            runCatching { module.deoptimize(m) }
            module.hook(m).intercept { chain ->
                ...
                if (forceLightDiag++ < 8) {
                    Log.i(
                        TAG,
                        "[diag] ThemeUtil.isNowThemeIsNight(" +
        ...
        if (overloads.isNotEmpty()) {
            logOnce("hook installed: ThemeUtil.isNowThemeIsNight x${overloads.size}")
        } else {
            logOnce("ThemeUtil.isNowThemeIsNight not found")
        }
        // ★ 真正的数据源：ThemeUtil 的两个"当前主题 id"读取。
        //   getUserCurrentThemeId(AppRuntime) 读 sp "currentThemeId_6.3.5"
        //   getCurrentThemeId()               读 getCurrentThemeInfo() 的 "themeId"
        // 它们返回 "1103"/"2920" 时 TIM 就认定"当前是夜间主题"，
        // isInNightMode / isNowThemeIsNight / isNowThemeDefaultNight 全由它派生。
        //
        // ⚠️ 这里必须用 **THEME_ID_DAYMODE_SIMPLE_WHITE = "2971"**，不能用 "1000"：
        //   ThemeUtil 的常量表（反编译 ThemeUtil.java L81-116）写得很清楚：
        //     DEFAULT_THEME_ID              = "1000"  // 只是"默认/未设置"，不是浅色主题
        //     THEME_ID_DAYMODE_SIMPLE_WHITE = "2971"  // ★ 浅色（DAYMODE = 日间模式）
        //     THEME_ID_DAYMODE_SIMPLE_GRAY  = "2921"  // 浅色（灰）
        //     THEME_ID_NIGHTMODE            = "1103"  // 夜间
        //     THEME_ID_NIGHTMODE_SIMPLE     = "2920"  // 夜间（SimpleUI）
        //   "1000" 会让 TIM 落回它自己的默认分支（可能仍是夜间资源），
        //   "2971" 才是明确"日间模式"的那一档。
        var idHooks = 0
        for (name in listOf("getUserCurrentThemeId", "getCurrentThemeId")) {
            val ms = runCatching { utilCls.declaredMethods }.getOrNull()
                ?.filter { it.name == name && it.returnType == String::class.java }
                .orEmpty()
            for (m in ms) {
                runCatching { module.deoptimize(m) }
                module.hook(m).intercept { THEME_ID_DAYMODE_SIMPLE_WHITE }
                idHooks++
            }
        }
        logOnce("hook installed: ThemeUtil themeId getters x$idHooks -> $THEME_ID_DAYMODE_SIMPLE_WHITE")
    } catch (t: Throwable) {
        Log.w(TAG, "ThemeUtil not found", t)
    }
}
...
private fun isJumpArrowDrawable(d: Drawable, host: View?): Boolean {
    ...
    val name = drawableNameMemo[d] ?: runCatching {
        d.constantState?.let { drawableStateNameMemo[it] }
    }.getOrNull()
    if (name != null) {
...
private val SUMMARY_BADGE_RE = Regex("^\\[?[0-9+]+条(新)?消息\\]?$")
...
private fun isExactSummaryBadge(t: String): Boolean = when (t) {
    ...
    "有新文件" -> true
    "[有新文件]" -> true
    else -> SUMMARY_BADGE_RE.matches(t)
}
...
private fun tintPillBackground(view: View?, bgColor: Int, createIfMissing: Boolean) {
    ...
    if (bg != null) {
        ColorMath.recolorInPlace(bg, bgColor)
...
/**
 * 角标/账号文本的 setText 时机处理（**已实现**；名字里的 "probe" 是历史遗留）。
 *
 * 处理两类短文本：
 * 1. 账号类文本（QQ 号等）→ 深色下改成不透明的 onSurface；
 * 2. 1~4 位纯数字（徽标数字晚 bind：attach 时文本为空，等 setText 写入后再判）。
 *
 * 原先这里的 KDoc 说"反编译正文丢失、仅为占位空实现"，与代码不符 —— 容易让人
 * 以为可以直接删掉这个调用点。实际两个分支都在生效。
 */
private fun probeWhiteNumberText(tv: TextView?, text: CharSequence?) {
    ...
    // 廉价预筛：关心的只有短文本（纯数字徽标 / QQ 号 / 摘要徽标），长文本
    // 直接跳过 —— 省掉每次 setText 都做一次 toString() 分配。
    val n = text.length
    if (n == 0 || n > 32) return
...
private fun hookSummaryBadge(module: XposedModule) {
    ...
    // TextView.setText(CharSequence)：单参变体。
    // ⚠️ 单参重载**内部会调用双参重载**，所以上面那个拦截器一定已经跑过
    // probeWhiteNumberText / remapDarkSpans / tintTodoRedText —— 这里再跑一遍
    // 等于每次 setText 做双份工作（会话列表最热路径）。
    // 只保留单参分支特有的 tintBadgeIfMatch（它需要 chain）。
        ...
        ?.let { method ->
            ...
            module.hook(method).intercept { chain ->
                ...
                if (name != null && name in badgeDrawables) {
                    ...
                    if (background != null) {
                        ColorMath.recolorInPlace(background, scheme.primary)
                    }
            ...
            ?.let { method ->
                ...
                module.hook(method).intercept { chain ->
                    ...
                    try {
                        ...
                        // ⚠️ 同样按名字取（不写死 0x7f060b6d = color/qui_common_feedback_error）
                        val errorColor = try {
                            val id = context.resources.getIdentifier(
                                "qui_common_feedback_error", "color", HOST_PACKAGE
                            )
                            if (id == 0) 0 else context.resources.getColor(id)
                        } catch (t: Throwable) {
                            0
                        }
    ...
    private fun hookProfileContentCard(module: XposedModule, cl: ClassLoader) {
        try {
            ...
            // 内容卡表面色：scheme.surfaceBright（AMOLED 黑时压成纯黑）
            val cardSurface = {
                MonetPalette.amoledBlack(MonetPalette.palette().surfaceBright)
            }
            // ⚠️ 必须**就地改色**，不能 `GradientDrawable()` 新建一个矩形顶上：
            // 新 drawable 没有 cornerRadius，会把卡片圆角削平（历史上就是这么丢的），
            // 而且丢掉原 drawable 的描边/多状态。返回同一个实例即可 —— dispatcher
            // 会把它当作 setBackground 的实参传下去，形状与颜色都是对的。
            val recolorCard = { d: Drawable ->
                ColorMath.recolorInPlace(d, cardSurface())
                d
            }
            // View.setBackground：资料卡激活期间，把“空背景/近白纯色底”替换成 surfaceBright
            onSetBackgroundArg { view, incoming ->
                if (profileUiCount <= 0) return@onSetBackgroundArg null
                // ⚠️ incoming == null 表示调用方要**清除背景**，绝不能当作"近白"替换成
                // 实色卡面 —— 那会把"清空"反转成一块 surfaceBright 矩形（实测多出方块底
                // 且丢圆角）。只在确实传入了近白纯色底时才替换。
                val d = incoming ?: return@onSetBackgroundArg null
                val nearWhite = solidColorOf(d)?.let {
                    BgResolver.isNearWhite(it or 0xFF000000.toInt())
                } ?: false
                if (nearWhite && isProfileContent(view, profileContentCls)) recolorCard(d) else null
            }
            ...
            onViewAttachedIf({ profileContentCls.isInstance(it) }) { view ->
                run {
                    ...
                    runCatching {
                        ...
                        // 强加等于给"本来无底"的容器凭空造一块矩形。
                        view.background?.let { recolorCard(it) }
                        // attach 即刷一次子树(延时兜底经实验证明非必需)
                        forceProfilePageCards(view)
            ...
            if (cellCls != null) {
                val cardBg = { TokenMapper.guestBubble() }
                onSetBackgroundArg { view, incoming ->
                ...
                findMethod(profileContentCls, setOf("rebuildProfileContent"), cardInfoType)
                    ?.let { method ->
                        ...
                        module.hook(method).intercept { chain ->
                            val result = chain.proceed()
                            try {
                                forceProfilePageCards(chain.thisObject as? View)
                            } catch (t: Throwable) {
    ...
    private fun tintSwitchThumb(drawable: Drawable?, color: Int) {
        ...
        // 容器：交给公共骨架递归展开到叶子
        if (drawable is DrawableContainer || drawable is LayerDrawable) {
            forEachContainerChild(drawable, recurse = true) { leaf -> tintSwitchThumb(leaf, color) }
            return
        }
        ColorMath.recolorInPlace(drawable, color)
        drawable.invalidateSelf()
                ...
                ?.let { method ->
                    ...
                    module.hook(method).intercept { chain ->
                        ...
                        runCatching {
                            ...
                            if (innerView != null) {
                                protectedUnreadViewIds.add(innerView)
                            }
                        }
                        val result = chain.proceed()
                        val bubble = try {
            ...
            findMethod(bubbleCls, setOf("X1"))
                ?.let { method ->
                    ...
                    module.hook(method).intercept { chain ->
                        ...
                        if (tv != null) {
                            tv.setTextColor(
                                MonetPalette.palette().onPrimary
                            )
                        }
                        result
                    }
    ...
    private fun forceProfilePageCards(view: View?) {
        if (view == null) return
    ...
    private fun recolorProfileCardContainer(container: DrawableContainer, scheme: DynamicScheme) {
        // **策略保留**：只染"最后一个非空子项"（TIM 把真正的卡面放在末位）。
        // 遍历本身交给公共骨架。
        var done = false
        forEachContainerChild(container, recurse = false) { child ->
            if (!done) {
    ...
    /** 通用容器：仅当子项为近白（luma >= NEAR_WHITE）纯色背景时替换为 surfaceBright。
     *  遍历交给 forEachContainerChild（不递归：子项自己再是容器时由 recolorDrawable 处理）。 */
    private fun recolorContainer(container: DrawableContainer, scheme: DynamicScheme) {
        forEachContainerChild(container, recurse = false) { child ->
            val solid = solidColorOf(child)
            if (solid != null && BgResolver.luma(solid or 0xFF000000.toInt()) >=
                BgResolver.NEAR_WHITE
            ) {
                recolorDrawable(child, scheme)
            }
        }
    ...
    /**
     * 红点判定取样缓存(drawable → 主色)；内容不变，与调色板无关。
     *
     * 键是 drawable 实例本身（`WeakHashMap`）：以前用 `identityHashCode` 存 Int，
     * GC 之后哈希会被新 drawable 复用，新图标会被当成"已取样"而跳过染色。
     * 详见 [ColorMath.instanceCache]。
     */
    private val redDotDominantMemo: MutableMap<Any, Int> = ColorMath.instanceCache()
    ...
    private fun glyphStats(drawable: Drawable): IntArray? {
        return try {
            ...
            for (c in px) {
                val a = (c ushr 24) and 0xFF
                if (a < BgResolver.ALPHA_MIN) continue
                core++
                val r = (c ushr 16) and 0xFF
                val g = (c ushr 8) and 0xFF
                val b = c and 0xFF
    ...
    private fun isHongbaoIconDrawable(drawable: Drawable): Boolean {
        val name = drawableNameMemo[drawable] ?: runCatching {
            drawable.constantState?.let { drawableStateNameMemo[it] }
        }.getOrNull() ?: return false
    ...
    private fun hookImageViewRedDot(module: XposedModule) {
        fun handleImage(view: ImageView?, drawable: Drawable?) {
            ...
            try {
                ...
                val domMemo = redDotDominantMemo[drawable]
                if (iw <= maxDotPx && ih <= maxDotPx && iw > 0 && ih > 0) {
                    if (drawable.colorFilter == null) {
                        if (domMemo != null) {
                            ...
                            if (dominant != null) {
                                redDotDominantMemo[drawable] = dominant
                            }
                        }
                    } else {
                        dominant = null
                ...
                val scheme = MonetPalette.palette()
                ColorMath.recolorInPlace(drawable, scheme.primary)
        ...
        findMethod(ImageView::class.java, setOf("setImageDrawable"), Drawable::class.java)
            ?.let { method ->
                ...
                module.hook(method).intercept { chain ->
                    val result = chain.proceed()
                    val iv = chain.thisObject as? ImageView
                    try {
                        handleImage(iv, chain.getArg(0) as? Drawable)
                    } catch (t: Throwable) {
                        // ignore
                    }
                    // 极光卡片（原 hookPolarLightCard，合并到这里 —— 同一个框架方法
                    // 不再挂两个 hook）：TIM 对它的背景渲染方式与普通卡片不同，
                    // 走通用图片染色会被映射到别的角色，实测设置页里正常卡片是卡片色
                    // 而"账号管理/账号安全"偏暗一档。这里按 view 类型统一成卡片色。
                    runCatching {
                        if (iv == null || !iv.javaClass.name.contains("PolarLight")) {
                            return@runCatching
                        }
                        if (!MonetPalette.isDarkNow()) return@runCatching
                        val d = iv.drawable ?: return@runCatching
                        d.mutate()
                        d.setColorFilter(TokenMapper.bgCard(), PorterDuff.Mode.SRC_IN)
                    }
                    result
                }
            }
    ...
    private fun forceQuiBadge(view: Any?) {
        ...
        try {
            ...
            val rowColor = findRowCardColor(view as? View, text)
            val bgColor: Int
            val fgColor: Int
            if (isRecall) {
                bgColor = rowColor
                ...
                bgColor = if (colorClose(rowColor, TokenMapper.bgCard())) {
                    scheme.secondaryContainer
                } else {
                    TokenMapper.guestBubble()
                }
            ...
            if (v != null && v.background != null) {
                ...
                ColorMath.recolorInPlace(viewBg, bgColor)
            }
    ...
    private fun findRowCardColor(view: View?, text: CharSequence): Int {
        if (view == null) return TokenMapper.bgList()
        // 文本变了说明这一行被复用给了别的会话，缓存必须失效
        val textHash = text.hashCode()
        badgeRowCache[view]?.let { if (it.first == textHash) return it.second }
        val listColor = TokenMapper.bgList()
        val cardColor = TokenMapper.bgCard()
        var color = listColor
        ...
        repeat(10) {
            ...
            when {
                ...
                isSurfaceColor(sample) && !sawAnySurface -> {
                    color = sample
                    sawAnySurface = true
                }
            }
        }
        badgeRowCache[view] = textHash to color
        return color
    }
    ...
    /** 该颜色是不是当前方案里的"面"（[isSchemeColor] 的子集）。转发 [BgResolver]。 */
    private fun isSurfaceColor(color: Int): Boolean = BgResolver.isSurfaceColorOf(color)
    ...
    private val protectedUnreadViewIds: MutableSet<View> = ColorMath.weakIdentitySet()
    ...
    private fun isUnreadBubbleView(view: View?): Boolean {
        ...
        if (view.javaClass.name.contains("UnreadBubble") ||
            view.javaClass.name.contains("unreadbubble") ||
            protectedUnreadViewIds.contains(view)
        ) {
            return true
        }
        ...
        while (current != null && depth < 10) {
            ...
            if (name.contains("UnreadBubble") || name.contains("unreadbubble") ||
                protectedUnreadViewIds.contains(current)
            ) {
                return true
            }
            current = current.parent as? View
    ...
    private fun protectUnreadSubtree(root: View?) {
        ...
        while (stack.isNotEmpty() && count < 80) {
            val current = stack.removeFirst()
            count++
            protectedUnreadViewIds.add(current)
            if (current is ViewGroup) {
                for (i in 0 until current.childCount) {
                    stack.addLast(current.getChildAt(i))
                }
        ...
        // 不再需要容量上限：条目随 View 被回收自动消失（见 ColorMath.weakIdentitySet）。
    }
    ...
    private fun fixLowContrastText(tv: TextView) {
        ...
        if (!dark) return
        if (!BgResolver.isGray(opaque)) return // 彩色文字不碰
    ...
    private fun isInsideBubble(v: View?): Boolean {
        ...
        val guestBg = ColorMath.opaque(TokenMapper.guestBubble())
            ...
            findMethod(cls, setOf("c"))
                ?.let { method ->
                    ...
                    module.hook(method).intercept { chain ->
                        ...
                        runCatching {
                            if (result != null && MonetPalette.isDarkNow()) {
                                val bg = TokenMapper.bgPage()
                                result.mutate()
                                result.setTint(bg)
    ...
    /** `View.setBackground(Drawable)` 的"入参替换"处理器。
     *
     *  返回非 null 表示替换这次设置的 drawable（第一个非 null 生效）。
     *  原来"资料卡强制卡片底"和"输入框强制圆角底"各挂一个 hook，同一个框架方法
     *  上叠了两层 chain.proceed + lambda，且先后顺序取决于安装顺序 —— 现在合并成
     *  一个 dispatcher + 注册表。 */
    private val bgArgReplacers = java.util.concurrent.CopyOnWriteArrayList<
        (View, Drawable?) -> Drawable?>()

    private fun onSetBackgroundArg(handler: (View, Drawable?) -> Drawable?) {
        bgArgReplacers.add(handler)
    }
    ...
    private fun fixTabIndicator(v: View) {
        runCatching {
            ...
            val primary = MonetPalette.palette().primary
    ...
    private fun fixThinBrightLineDrawable(v: View, d: Drawable) {
        ...
        // ⚠️ 与其他所有"亮底压暗"路径保持一致：**就地改色**，绝不手工 new 一个
        // GradientDrawable 顶上 —— 那会丢掉原 drawable 的圆角/九宫格留白/描边/
        // 多状态（"卡片圆角被削平"就是这么来的）。详见 ColorMath.recolorInPlace。
        ColorMath.recolorInPlace(d, TokenMapper.bgPage())
        logOnce(
     ...
     *  （tintAnyDrawable / fixTinySolidBg）是在**之后**才跑的 —— 实测会把专用
     *  结果覆盖成暗一档（设置页"账号管理/账号安全"= #380E33，其它卡片 #4A1843，
     *  排除了 PolarLight 也无效，因为覆盖发生在后面）。
     *  这里在 attach 之后再补染一次，让专用结果成为最终结果。 */
    private fun hookPolarLightLate() {
        onViewAttached { v0 ->
            runCatching {
     ...
     *  两条门槛都放过），是残留亮条的主要来源。 */
    /** View 是否"几乎占满屏宽"（默认 90%）。
     *
     *  ⚠️ 以前这类判断写死成 `width >= 1250` —— 那是按 1344px 宽的设备标定的：
     *  在 1080p 手机上不生效，在平板（2120~3000px）上又会命中一切（实测把设置页
     *  的"手机号码"行当成了全宽的"微云入口条"）。改用**屏宽比例**，与设备无关。 */
    private fun isFullWidth(v: View, ratio: Float = 0.9f): Boolean {
        val screenW = v.resources.displayMetrics.widthPixels
        return screenW > 0 && v.width >= (screenW * ratio).toInt()
    }

    /** dp -> px。用当前 View 的 density（平板上常有 density override）。 */
    private fun View.dpPx(value: Float): Int =
        (value * resources.displayMetrics.density).toInt()

    /** "这个 View 的背景就是图标本身"。
     *
     *  TIM 常用 ImageButton / ImageView 的 **background** 来画图标（drawable 为
     *  null，图在 StateListDrawable 里）。把这种背景当"底色"处理（压暗 / 换面色）
     *  会让图标变色甚至消失 —— 实测聊天页输入框右侧的表情/加号两个 ImageButton
     *  在暗色下消失，就是被 fixTinySolidBg 当成"浅色遗留底"压成了页面底色。
     *
     *  所有"把 background 当作色块"的规则都应先过这一关。 */
    ...
    private fun fixTinySolidBg(v: View, d: Drawable) {
        ...
        // 不能在这里再被"亮背景压暗"处理 —— 那会把它压成**页面底色**，导致
        // 同一张卡片里的各个 item 颜色不一致（实测设置页"账号管理 / 手机号码 /
        // 账号安全"变成了 #4A1843 / #380E33 / #30092B 三种色）。
        // 它的背景本来就是浅色渐晕图，不是"浅色主题遗留的亮底"。
        if (v.javaClass.name.contains("PolarLight")) return
        // ⚠️ ImageButton / 没有 drawable 的 ImageView：它们的 background 往往就是
        // **图标本身**（TIM 用 StateListDrawable 画图标，drawable 为 null）。
        // 把这种背景当"浅色主题遗留的亮底"压暗，图标就被染成页面底色 —— 在深色
        // 页面上直接"消失"。实测聊天页输入框右侧的表情/加号两个 ImageButton 就是
        // 这样没的（探针：ImageButton vis=0 42x42 a=1.0 d=null bg=StateListDrawable）。
        if (bgIsIcon(v)) return
        // 判据彻底放开，只留一条本质规则：
        ...
        val targetBg = if (isInputFieldHost(v)) TokenMapper.inputBg()
        else TokenMapper.bgPage()
    ...
    private fun fixNoticeBar(v: View) {
        ...
        if (noticeBarPainted.add(v)) {
            // ⚠️ 就地改色，不要 new 一个矩形 GradientDrawable 顶上：
            // ① 已经确认它的背景是**皮肤引擎的九宫格**，换成普通矩形会丢掉
            // 九宫格的拉伸留白与 insets（提示条文字贴边/变形），而且与
            // fixTinySolidBg / fixThinBrightLine 的做法自相矛盾。
            v.background?.let { ColorMath.recolorInPlace(it, TokenMapper.bgPage()) }
        }
     ...
     *  不用等到 onResume 之后。 */
    private fun hookAttachedNoticeBar() {
        onViewAttached { v ->
    ...
    private fun hookViewBackground(module: XposedModule) {
        ...
        findMethod(View::class.java, setOf("setBackground"), Drawable::class.java)
            ?.let { hookFrameworkMethod(module, it) { chain, result ->
                ...
                val view = chain.thisObject as? View
                // 极小/亮色纯色位图背景的兜底（原 hookSetBackground 的后置处理）。
                // 合并到这里是为了**同一个框架方法只挂一个 hook** —— 以前这里和
                // hookSetBackground 各挂一个，热路径多层 chain，且执行顺序取决于
                // 安装顺序。
                (chain.getArg(0) as? Drawable)?.let { d0 ->
                    if (view != null) runCatching { fixTinySolidBg(view, d0) }
                }
                ...
                val drawable = chain.getArg(0) as? Drawable
                val dark = MonetPalette.isDarkNow()
                if (drawable != null) {
                    ...
                    if (!tintAnyDrawable(drawable, dark)) {
                        ...
                        if (dname == "com.tencent.mobileqq.login.fragment.p" ||
                            ...
                            if (scheme.isDark) {
                                ...
                                runCatching {
                                    ColorMath.recolorInPlace(drawable, tgt)
        ...
        findMethod(View::class.java, setOf("setForeground"), Drawable::class.java)
            ?.let { hookFrameworkMethod(module, it) { chain, result ->
                ...
                val drawable = chain.getArg(0) as? Drawable
                if (drawable != null) {
                    tintAnyDrawable(drawable, MonetPalette.isDarkNow())
                }
                null
            } }
            ...
            ?.let { method ->
                ...
                module.hook(method).intercept { chain ->
                    ...
                    if (alpha >= 0xE0 && isErrorRed(color or 0xFF000000.toInt())) {
                        mapped = MonetPalette.palette().primary
                        ...
                        mapped = if (isSchemeColor(color)) {
                            color
                        } else {
                            TokenMapper.inlineBgColor(color) ?: color
                        }
                    }
    ...
    /**
     * "这个 drawable 在当前调色板代次里已经染过"的标记（drawable → 代次）。
     *
     * ⚠️ 键必须是 drawable **实例**而不是它的 identityHashCode：用 Int 做键时，
     * drawable 被回收后哈希会被新对象复用，新 drawable 会被当成"已染过"直接
     * `return true` 跳过 —— 表现是"偶尔有个控件不变色"，随 GC 时机变化。
     * 弱引用键随 drawable 一起消失，也不需要容量上限。详见 [ColorMath.instanceCache]。
     */
    ...
    private fun tintAnyDrawable(drawable: Drawable, dark: Boolean): Boolean {
        ...
        val gen = MonetPalette.generation()
        drawableTintMemo[drawable]?.let { if (it == gen) return true }
        val result = tintAnyDrawableImpl(drawable, dark)
        if (result) {
            drawableTintMemo[drawable] = gen
        }
        return result
    }
    ...
    private fun tintAnyDrawableImpl(drawable: Drawable, dark: Boolean): Boolean {
        ...
        if (drawable is ColorDrawable) {
            ...
            if (isSchemeColor(original)) return true
            val mapped = TokenMapper.bgColorForDrawable(original) ?: return false
            if (mapped != original) {
                drawable.mutate()
        ...
        if (drawable is GradientDrawable) {
            ...
            if (solid != null) {
                if (isSchemeColor(solid)) return true
                val mapped = TokenMapper.bgColorForDrawable(solid) ?: return false
                if (mapped != solid) {
                    drawable.mutate()
            ...
            for (i in colors.indices) {
                val c = colors[i]
                val m = if (isSchemeColor(c)) {
                    c
                } else {
                    TokenMapper.bgColorForDrawable(c) ?: c
                }
                out[i] = m
                if (m != c) changed = true
            }
        ...
        if (drawable is android.graphics.drawable.BitmapDrawable) {
            ...
            val mapped = TokenMapper.bgColorForDrawable(dominant) ?: return false
            if (mapped != dominant) {
                drawable.mutate()
        ...
        if (drawable is android.graphics.drawable.NinePatchDrawable) {
            ...
            val mapped = TokenMapper.inlineBgColor(dominant) ?: return false
            if (mapped != dominant) {
                drawable.mutate()
                drawable.colorFilter = PorterDuffColorFilter(mapped, PorterDuff.Mode.SRC_IN)
                drawable.invalidateSelf()
    ...
    /**
     * 该颜色是不是"当前配色方案里我们已经输出过的颜色"——转发 [BgResolver]。
     *
     * 角色清单只此一份（原先 hooks 与 TokenMapper 各抄一遍，加角色时常只改一处）。
     * ⚠️ **没有** `dark` 形参：历史上它存在但从不参与判据，深浅一律取自
     * `MonetPalette.palette()`；留着只会让人以为传错深浅会改变结果。
     */
    private fun isSchemeColor(color: Int): Boolean = BgResolver.isSchemeColor(color)
    ...
    private fun redSkinFilterOf(paint: Paint?, drawable: Drawable): Int {
        ...
        val src = try {
            // 这个字段名跨版本改过三次，必须都试（实测 Android 17 上是 mAdd/mMul）：
            //   API < 29 : mSrcColor / mDstColor + PorterDuff.Mode
            //   API 29~34: mColor + BlendMode
            //   Android 15+ / 17: mAdd + mMul（颜色矩阵形式，单色滤色时 mMul 即目标色）
            // 只找老名字的话会静默返回 0（"有滤色但非红"）→ 皮肤红气泡永远不改色。
            val f = cachedField(filter.javaClass, "mSrcColor")
                ?: cachedField(filter.javaClass, "mColor")
                ?: cachedField(filter.javaClass, "mMul")
        ...
        if (src == null) {
            if (redFilterDiag++ < 3) {
                Log.i(
                    TAG,
                    "red skin filter: no color field; fields=" +
                        filter.javaClass.declaredFields.joinToString { it.name }
                )
            }
            return 0
        }
    ...
    private fun mappedBitmapBgColor(dominant: Int): Int? {
        val opaque = opaqueColor(dominant)
        if (isErrorRed(opaque)) {
            return MonetPalette.palette().primary
        }
        return TokenMapper.inlineBgColor(opaque)
    }
        ...
        return try {
            when (drawable.javaClass.name) {
                "com.tencent.theme.SkinnableBitmapDrawable" -> {
                    ...
                    val mapped = mappedBitmapBgColor(dominant) ?: return false
                ...
                "com.tencent.theme.SkinnableNinePatchDrawable" -> {
                    ...
                    if (paint != null) {
                        ...
                        ColorMath.recolorInPlace(drawable, mapped)
                    }
                    drawable.invalidateSelf()
                    ...
                    module.hook(m).intercept { chain ->
                        ...
                        runCatching {
                            ...
                            val fg = cachedField(cls, "mPaintForeground")?.also { it.isAccessible = true }
                                ?.get(chain.thisObject) as? Paint
                            val scheme = MonetPalette.palette()
                            bg?.setColor(TokenMapper.guestBubble())
                            fg?.setColor(scheme.primary)
                            if (payPwdLog++ < 8) {
                                Log.i(
                                    TAG,
                                    "pay pwd paints -> grid #" +
                                        Integer.toHexString(TokenMapper.guestBubble()) +
        ...
        module.hook(mm).intercept { chain ->
            ...
            when {
                c == PAY_PWD_GRID_COLOR -> {
                    ...
                    val mapped = TokenMapper.guestBubble()
                    if (payPwdLog++ < 4) {
                ...
                // 圆点的原值（仅供查证）：
                //   DownloadCardView.COLOR_BUTTON_BACKGROUND_DARK = -13421773 = #FF333333
                else -> chain.proceed()
            }
        }
    }
...
private fun fixAvIconTint(csl: ColorStateList): ColorStateList? {
    ...
    val fixed = IntArray(colors.size) { i ->
        ...
        if (BgResolver.isSurfaceColorOf(c) || c == TokenMapper.bgPage()) {
            changed = true
...
private fun hookAioEditText(module: XposedModule, cl: ClassLoader) {
        ...
        onSetBackgroundArg { view, _ ->
            if (!cls.isInstance(view)) return@onSetBackgroundArg null
            val density = view.resources.displayMetrics.density
        ...
        val hintColor: () -> Int = {
            MonetPalette.palette().onSurfaceVariant
        }
        findMethod(TextView::class.java, setOf("setHintTextColor"), INT_TYPE)
            ?.let { method ->
        ...
        findMethod(cls, setOf("f"), Context::class.java)
            ?.let { method ->
                ...
                module.hook(method).intercept { chain ->
                    val result = chain.proceed()
                    if (result is Int) {
                        MonetPalette.palette().onSurface
                    } else {
                        result
                    }
                }
    ...
    private fun tintSingleLineBadgeIcon(view: Any?, color: Int) {
        ...
        try {
            ...
            ColorMath.recolorInPlace(left, color)
            left.invalidateSelf()
            v.invalidate()
        } catch (t: Throwable) {
    ...
    /**
     * BT.601 亮度 —— 转发 [BgResolver.luma]。
     *
     * ⚠️ 不要再在这里写第二份实现：项目里曾经同时存在 hooks 的 colorLuma、
     * TokenMapper 的 luma 和 BgResolver 的 luma 三份同样的式子，改阈值时
     * 很容易只改一处。
     */
    private fun colorLuma(color: Int): Int = BgResolver.luma(color)

    /** 同上，但直接吃分量 —— 供像素循环使用，省掉为了调用而打包成 Int。 */
    private fun colorLuma(r: Int, g: Int, b: Int): Int = BgResolver.luma(r, g, b)

    // ---- 亮度阈值 ----
    // 这些判定线历史上各处各写各的（120/140/150/160/200/220/235），语义其实
    // 只有下面几类。收敛成命名常量，**取值一个没改**，只是让"为什么是 170"
    // 变得可读，将来要统一调整也只改一处。
    // 亮度档位统一由 BgResolver 定义（这里保留旧名字作为别名，调用点无需改动）
    private const val LUMA_VERY_LIGHT = BgResolver.VERY_LIGHT
    private const val LUMA_LIGHT = BgResolver.LIGHT
    private const val LUMA_BRIGHT_BG = BgResolver.BRIGHT
    private const val LUMA_MID = BgResolver.MID
    private const val LUMA_MID_LOW = BgResolver.MID_LOW
    private const val LUMA_DARK_TEXT = BgResolver.DARK_TEXT
    private const val LUMA_DARK = BgResolver.DARK
                ...
                module.hook(method).intercept { chain ->
                    ...
                    try {
                        ...
                        while (keys.hasNext()) {
                            ...
                            var mapped = mapTokenString(key, json.optString(key))
                            ...
                            if (dark &&
                                ...
                                mapped = mapped.split(",").joinToString(",") { part ->
                                    ...
                                    if (color != null && BgResolver.isNearWhite(color)) {
                                        colorToHex(TokenMapper.bgCard())
                                    } else {
                                        part
                                    }
                                }
    ...
    /** 已扫描过的文件气泡宿主。
     *  ⚠️ 以前是 ConcurrentHashMap.newKeySet<Int>() 存 identityHashCode，且在
     *  size > 32 时整体 clear()（连刚加入的自身也清掉，去重同时失效）。
     *  改成弱引用键集合：对象回收即自动移除，不需要手工淘汰。 */
    private val fileMonitored: MutableSet<View> =
        java.util.Collections.synchronizedSet(
            java.util.Collections.newSetFromMap(java.util.WeakHashMap<View, Boolean>())
        )

    private fun hookFileDownloadIcons() {
        runCatching {
            onViewAttached { view ->
                val name = view.javaClass.name
                if ((name.contains("AIOFile") || name.contains("aiofile")) &&
                    fileMonitored.add(view)
                ) {
    ...
    private fun scanFileIcons(root: View?) {
        ...
        try {
            ...
            while (stack.isNotEmpty() && guard < 400) {
                ...
                if (dominant == null || !BgResolver.isNearWhite(dominant)) continue
                    ...
                    try {
                        ...
                        if (csl != null && !csl.isStateful) {
                            ...
                            if (scheme.isDark) {
                                ...
                                val luma = colorLuma(opaque)
                                if (grayish && luma < LUMA_BRIGHT_BG && !isSchemeColor(c)) {
        ...
        findMethod(TextView::class.java, setOf("setTextColor"), INT_TYPE)
            ?.let { method ->
                ...
                module.hook(method).intercept { chain ->
                    ...
                    val tvScheme = MonetPalette.palette()
                ...
                ?.let { method ->
                    ...
                    module.hook(method).intercept { chain ->
                        ...
                        try {
                            ...
                            val mapped = TokenMapper.mapColor(name, tv.currentTextColor)
            ...
            findMethod(cls, setOf(methodName), Context::class.java)
                ?.let { method ->
                    ...
                    module.hook(method).intercept { chain ->
                        val result = chain.proceed()
                        if (result is Int) {
                            TokenMapper.mapColor(tokenName, result)
                        } else {
                            result
                        }
                    }
    ...
    private fun hookDrawables(module: XposedModule) {
        ...
        // 注意用严格版：该签名下若没有 loadDrawable，兜底会抓到 loadColorStateList
        // （返回值是 ColorStateList，被 `as? Drawable` 变成 null，纯属挂错）。
        findMethodStrict(
            resourcesCls,
            setOf("loadDrawable"),
            TypedValue::class.java,
    ...
    /** 暖色单色小图形判定缓存（drawable → 是否暖色）。键为实例，见 [ColorMath.instanceCache]。 */
    private val warmGlyphMemo: MutableMap<Any, Boolean> = ColorMath.instanceCache()
    ...
    private fun tintDrawable(name: String?, drawable: Drawable): Drawable {
        ...
        if (name.contains("qui_header_tab_")) {
            ...
            val line = TokenMapper.outlineVariant()
            val gd = GradientDrawable()
            gd.shape = GradientDrawable.RECTANGLE
            gd.setColor(if (sel) line else TokenMapper.bgPage())
        ...
        if (name == "dwa") {
            ...
            ColorMath.recolorInPlace(drawable, TokenMapper.bgPage())
            if (payPwdLog++ < 4) {
        ...
        val color = TokenMapper.tintColorFor(name)
        if (color == null) {
            ...
            if (drawable is ColorDrawable || drawable is GradientDrawable) {
                if (tintAnyDrawable(drawable, MonetPalette.isDarkNow())) {
    ...
    /** 已确认"染不了"的图标（彩色 / 尺寸不合适）。
     *  彩色图标每次都会走完「分配 Bitmap + draw + getPixels + 两轮像素扫描」才
     *  return null，而它们**永远**染不了 —— 面板/列表里每帧重来一遍是纯浪费。
     *  这些结论与配色无关，不需要 generation。 */
    private val rasterizeDenied: MutableSet<Drawable> =
        java.util.Collections.synchronizedSet(
            java.util.Collections.newSetFromMap(java.util.WeakHashMap<Drawable, Boolean>())
        )

    private fun rasterizeIconUniform(drawable: Drawable): Drawable? {
        if (rasterizeDenied.contains(drawable)) return null
        return try {
            ...
            if (drawable.javaClass.name.contains("URL", ignoreCase = true)) {
                val urlScheme = MonetPalette.palette()
                if (urlScheme.isDark) {
            ...
            if (iw <= 0 || ih <= 0 || iw > 240 || ih > 240) {
                rasterizeDenied.add(drawable)
                return null
            }
    ...
    private fun tintInlineDrawable(drawable: Drawable, typedValue: TypedValue?): Drawable {
        ...
        val mapped = TokenMapper.inlineBgColor(inlineColor)
            ?: return drawable
        return try {
            if (drawable is android.graphics.drawable.ColorDrawable) {
                drawable.color = mapped
    ...
    private fun cachedField(cls: Class<*>?, name: String): Field? {
        ...
        fieldCache[key]?.let { return if (it === cacheMiss) null else it as Field }
        val field = findFieldDeep(cls, name)
        fieldCache[key] = field ?: cacheMiss
    ...
    private fun cachedFieldByType(cls: Class<*>?, type: Class<*>): Field? {
        ...
        typeFieldCache[key]?.let { return if (it === cacheMiss) null else it as Field }
        val field = findFieldByTypeDeep(cls, type)
        typeFieldCache[key] = field ?: cacheMiss
                ...
                ?.let { method ->
                    ...
                    hookFrameworkMethod(module, method) { chain, result ->
                        ...
                        if (d is ColorDrawable || d is GradientDrawable) {
                            if (tintAnyDrawable(d, MonetPalette.isDarkNow())) {
            ...
            ?.let { hookFrameworkMethod(module, it) { chain, result ->
                ...
                if (result is Int) {
                    ...
                    if (type == TypedValue.TYPE_STRING) {
                        val dark = MonetPalette.isDarkNow()
                        val mapped = TokenMapper.inlineBgColor(result)
                        ...
                        when {
                            ...
                            dark -> TokenMapper.inlineBgColor(result) ?: result
                            else -> result
                        }
                    }
                } else {
    ...
    private fun darkTextFallback(color: Int): Int? {
        val scheme = MonetPalette.palette()
        if (!scheme.isDark) return null
    ...
    private fun remapColorByName(name: String?, color: Int): Int {
        ...
        if (page != null && page.contains("GesturePWD") && name != null &&
            ...
            return TokenMapper.outlineVariant()
        }
        ...
        if (!knownName) {
            ...
            TokenMapper.inlineBgColor(color)?.let { return it }
        ...
        val mapped = TokenMapper.mapColor(name, color)
    ...
    private fun entryName(resources: Resources?, resId: Int): String? {
        ...
        entryNameCache[key]?.let { return if (it === cacheMiss) null else it as String }
        val name = try {
            resources.getResourceEntryName(resId)
        } catch (t: Throwable) {
            null
        }
        entryNameCache[key] = name ?: cacheMiss
     ...
     *
     * ⚠️ 必须线程安全：`hookQuiTokenManager` / `hookQuiUtil` 的 after 钩子会跑在
     * 任意线程上，裸 `HashMap` 并发写会丢更新。代次也从这里读写，同样要 volatile。
     *
     * 已知取舍：容量超限时整体 `clear()`，会连刚写入的那一条一起丢掉（下次重算）。
     * 这里刻意保留这个行为 —— 只影响 1 条、代价是一次重算，比引入 LRU 更简单。
     */
    private val tokenColorMemo = java.util.concurrent.ConcurrentHashMap<Int, Int>()
            ...
            ?.let { method ->
                ...
                module.hook(method).intercept { chain ->
                    ...
                    try {
                        ...
                        TokenMapper.mapColor(name, result)
                    } catch (t: Throwable) {
                ...
                module.hook(method).intercept { chain ->
                    ...
                    try {
                        ...
                        for ((key, value) in map) {
                            ...
                            out[key] = mapTokenString(key, value)
                        }
    ...
    private fun mapTokenString(key: String, value: String): String {
        ...
        val mapped = parts.map { part ->
            ...
            colorToHex(TokenMapper.mapPageToken(key, color))
        }
    ...
    private fun remapColorStateList(
        ...
        val mapped = IntArray(colors.size) { index ->
            TokenMapper.mapColor(name, colors[index])
        }
    ...
    private fun findMethod(
        ...
        val declared = runCatching { cls.declaredMethods }.getOrNull()
            ?: run {
                logOnce("findMethod: no declaredMethods on " + cls.name)
                return null
            }
        for (name in names) {
        ...
        // ⚠️ 兜底：按参数类型找，**不校验名字与返回类型** —— TIM 一改混淆名就可能
        // 命中"该类第一个同参数的方法"，而有些拦截器丢弃 proceed 结果返回字面
        // null/false，一旦挂到非 void 方法上就会把 null 交给调用方。
        // 以前这里是完全静默的（升级 TIM 后随机失效且无任何线索），现在留痕。
        val fallback = declared.firstOrNull { Arrays.equals(it.parameterTypes, paramTypes) }
        if (fallback != null) {
            logOnce(
                "findMethod FALLBACK " + cls.simpleName + ": wanted=" + names +
                    " got=" + fallback.name + ":" + fallback.returnType.simpleName
            )
            return fallback.also { runCatching { it.isAccessible = true } }
        }
        logOnce(
            "findMethod MISS " + cls.simpleName + ": wanted=" + names +
                " params=(" + paramTypes.joinToString { it.simpleName } + ")"
        )
        return null
    }
    ...
    /** **严格版**：不做按参数类型的兜底匹配。
     *
     *  语义敏感的方法（show/dismiss、loadDrawable/loadColorStateList…）必须用它 ——
     *  否则 TIM 一改混淆名，兜底就会静默挂到同参数的邻居方法上。实测
     *  QQCustomArkDialogForAio 的 show 就被挂成了 dismiss（弹窗关闭时才染背景）。 */
    private fun findMethodStrict(
        cls: Class<*>,
        names: Set<String>,
        vararg paramTypes: Class<*>
    ): Method? {
    ...
    /** 严格版 + 沿父类向上找（用于 show 这类继承自 Dialog 的方法）。 */
    private fun findMethodStrictDeep(
        cls: Class<*>,
        names: Set<String>,
        vararg paramTypes: Class<*>
    ): Method? {
        ...
        while (c != null && c != Any::class.java) {
            findMethodStrict(c, names, *paramTypes)?.let { return it }
            c = c.superclass
        }
        logOnce("findMethodStrictDeep MISS " + cls.simpleName + ": wanted=" + names)
        return null
    }
}
